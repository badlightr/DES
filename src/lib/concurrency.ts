...
export async function checkAndPreventOverlap(
  ...
  const realQuery = query.replace(/start_at/g, "start_time").replace(/end_at/g, "end_time");
...
export async function validateOvertimeRequest(
  ...
  const currentDayTotal = (dayTotal._sum.total_minutes || 0) + durationMin;
  ...
  const currentWeekTotal = (weekTotal._sum.total_minutes || 0) + durationMin;
